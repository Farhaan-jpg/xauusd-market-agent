...
<i>Implied rate cut probabilities derived from Treasury curve differentials.</i>""".strip()
    ...
    @staticmethod
    def trade_setup_broadcast_template(setup_payload: dict) -> str:
        """Formats an actionable, multi-factor verified trade setup for Telegram broadcast."""
        esc = TelegramBot.escape
        mode = setup_payload.get("mode", "SCALPING")
        dir_val = setup_payload.get("direction", "BULLISH").upper()
        is_bull = "BULL" in dir_val
        dir_emoji = "🟢" if is_bull else "🔴" if "BEAR" in dir_val else "⚪"
        action = "BUY / LONG" if is_bull else "SELL / SHORT" if "BEAR" in dir_val else "RANGE ACCUMULATION"

        setup = setup_payload.get("actionable_setup", {})
        price = setup_payload.get("current_price", 2724.50)
        entry = setup.get("entry_price", price)
        sl = setup.get("stop_loss", price - 4.5 if is_bull else price + 4.5)
        sl_pts = setup.get("stop_loss_pts", abs(entry - sl))
        tp1 = setup.get("take_profit_1", entry + 8.5 if is_bull else entry - 8.5)
        tp2 = setup.get("take_profit_2", entry + 16.0 if is_bull else entry - 16.0)
        rr = setup.get("risk_reward_ratio", "1:2.5")
        horizon = setup.get("holding_horizon", "5-30 Mins" if mode == "SCALPING" else "4-18 Hours")
        score = setup_payload.get("composite_score", 18.5)
        conviction = setup_payload.get("conviction_pct", 85)
        thesis = setup_payload.get("executive_thesis", "Multi-factor convergence across yields, safe-haven flows, and structure.")

        return f"""⚜️ <b>XAUUSD {mode} TRADE SETUP</b> {dir_emoji}
📅 <i>{get_formatted_time()}</i>
━━━━━━━━━━━━━━━━━━━━
🎯 <b>ACTION:</b> <b>{action}</b> ({setup.get('style', 'Momentum FVG')})
⭐️ <b>QUALITY GRADE:</b> <b>A+ INSTITUTIONAL</b> ({conviction}% Conviction)
💰 <b>CURRENT SPOT:</b> <b>${price:.2f}</b>

📊 <b>TRADE EXECUTION MATRIX:</b>
• 🟢 <b>Entry Zone:</b> <b>${entry:.2f}</b>
• 🛑 <b>Invalidation SL:</b> <b>${sl:.2f}</b> (Risk: {sl_pts:.1f} pts)
• 🎯 <b>Take Profit 1:</b> <b>${tp1:.2f}</b> (Scale 50% + SL to BE)
• 🚀 <b>Take Profit 2:</b> <b>${tp2:.2f}</b> (Runner target)
• ⚖️ <b>Risk:Reward Ratio:</b> <b>{rr}</b>
• ⏱ <b>Holding Horizon:</b> <b>{horizon}</b>
━━━━━━━━━━━━━━━━━━━━
🧠 <b>CONFLUENCE RATIONALE (Score: {score:+.1f}):</b>
{esc(thesis)}

📋 <b>MANAGEMENT ROADMAP:</b>
1. Fill limit/market in designated zone with 1.0R risk.
2. Close 50% position at TP1 and immediately shift SL to Entry (Breakeven).
3. Trail remainder into TP2 liquidity pool.
━━━━━━━━━━━━━━━━━━━━
<i>Generated by XAUUSD AI Intelligence & Market Direction Platform.</i>""".strip()
